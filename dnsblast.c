...
{
    printf("      ER288 <-----     ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%ld MAX_PACKETS:%s ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,context->ai->ai_canonname);
    while (receive(context) == 0)
        ;
    printf("      ER292 <-----     ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%ld MAX_PACKETS:%s ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,context->ai->ai_canonname);
