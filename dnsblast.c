...
{
    printf("      ER288 <-----     ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%s MAX_PACKETS:%lld ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,context->ai->ai_canonname);
    while (receive(context) == 0)
        ;
    printf("      ER292 <-----     ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%s MAX_PACKETS:%lld ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,context->ai->ai_canonname);
...
throttled_receive(Context * const context)
{
    ...
    printf("    TR306 <-----       ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%ld MAX_PACKETS:%s ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,max_packets,elapsed);
        empty_receive_queue(context);
    printf("    TR308 <-----       ID:%d SENDING:%d SENT_PACKETS:%ld RECEIVED_PACKETS:%ld MAX_PACKETS:%s ELAPSED:%lld\n",context->id,context->sending,context->sent_packets,context->received_packets,max_packets,elapsed);
    }
