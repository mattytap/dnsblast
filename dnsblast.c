...
blast(Context * const context, const char * const name, const uint16_t type)
{
    ...
    printf("            D117  ID:%d SOCK:%d QUETION:%s MSG:%s TYPE:%d NAME:%s SENT_PACKETS:%ld RECEIVED_PACKETS:%ld PACKET_SIZE:%ld----->\n",context->id,context->sock,question,msg,type,name,context->sent_packets,context->received_packets,packet_size);
    }
    context->sent_packets++;
    printf("        C120      ID:%d SOCK:%d QUETION:%s MSG:%s TYPE:%d NAME:%s SENT_PACKETS:%ld RECEIVED_PACKETS:%ld PACKET_SIZE:%ld----->\n",context->id,context->sock,question,msg,type,name,context->sent_packets,context->received_packets,packet_size);
